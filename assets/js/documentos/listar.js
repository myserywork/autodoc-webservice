...
var searchConvenio = $("#searchConvenio").val();
...
if (urlParams.has("searchConvenio")) {
	searchConvenio = urlParams.get("searchConvenio");
	$("#searchConvenio").val(searchConvenio);
}
...
$(document).ready(function () {
    ...
    $("#searchConvenio").keyup(
		debounce(function () {
			page = 1;
			searchConvenio = $("#searchConvenio").val();
			carregarCards();			
		}, 650)
	);
...
function populateDivpaginationContainer(totalPages, currentPage) {
	...
	if (Number(currentPage) != 1) {
		...
		previousButton.addEventListener("click", () => {
			...
			let url = base_url + "documentos?page=" + previousPage;
			if (searchNome) {
				url += "&searchNome=" + encodeURIComponent(searchNome);
			}
			if (searchConvenio) {
				url += "&searchConvenio=" + encodeURIComponent(searchConvenio);
			}
	...
	for (let i = startPage; i <= endPage; i++) {
        ...
        let url = base_url + "documentos?page=" + i;
        if (searchNome) {
            url += "&searchNome=" + encodeURIComponent(searchNome);
        }
        if (searchConvenio) {
	...
	if (Number(currentPage) != totalPages) {
		...
		nextButton.addEventListener("click", () => {
            ...
            let url = base_url + "documentos?page=" + nextPage;
			if (searchNome) {
				url += "&searchNome=" + encodeURIComponent(searchNome);
			}
			if (searchConvenio) {
...
function carregarCards() {
    ...
    const base_url = $('#base_url').val();
    var apiUrl = base_url + 'documentos/getDocumentos?page=' + page;  
	...
	if (searchConvenio) {
		apiUrl += "&searchConvenio=" + searchConvenio;
	}
        ...
        .then(data => {
            ...
            data.data.forEach(item => {
                ...
                <div class="card-body card-documentos">
					<h5 class="card-title mb-1 convenio-card-state"><i class="fa fa-sitemap" aria-hidden="true"></i> Nº Convênio: ${item.id_convenio}</h5>
